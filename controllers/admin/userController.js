    ...
    if (req.body.password === req.body.cpassword) {
        if (schema.validate(req.body.password)) {
            bcrypt.hash(req.body.password, 12, async (error, hash) => {
                if (error) {
                    ...
                    try {
                        ...
                        error.errors?.phone ? errorMessage.phone = error.errors.email.message : null;
            ...
            res.render("admin/user/create", {
                title: "Admin - Create User",
                errorMessage: {
                    password: "Invalid Password. It Must Contain at least 1 upper case and 1 lower case alphabet, 1 digit, should not contain any space and length must be 8-100"
                },
                data: data,
                session: req.session
            })
...
async function updateProfileStore(req, res) {
    try {
        ...
        let errorMessage = {}
        error.keyValue?.username ? errorMessage.username = "Username Already Exist" : null,
            error.keyValue?.email ? errorMessage.email = "Email Address Already Exist" : null,
            error.errors?.name ? errorMessage.name = error.errors.name.message : null;
        error.errors?.username ? errorMessage.username = error.errors.username.message : null;
        error.errors?.email ? errorMessage.email = error.errors.email.message : null;
        error.errors?.phone ? errorMessage.phone = error.errors.email.message : null;
        ...
        if (data) {
            if (req.body.password === req.body.cpassword) {
                if (schema.validate(req.body.password)) {
                    ...
                    res.render("admin/user/forget-password3", {
                        title: "Admin - Reset Password",
                        errorMessage: "Invalid Password. It Must Contain at least 1 upper case and 1 lower case alphabet, 1 digit, should not contain any space and length must be 8-100"
                    })
            }
            else
                res.render("admin/user/forget-password3", {
